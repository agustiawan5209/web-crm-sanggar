...
function deleteItem() {
    DeleteForm.delete(route('Customer.destroy'), {
        preserveState: true,
        preserveScroll: true,
        onFinish: () => {
...
const UpdateStatusCustomer = ref(false);
function showPembayaranModal(item) {
    FormStatus.slug = item.id;
    FormStatus.status = item.status;

    UpdateStatusCustomer.value = true;
}
...
function submitStatus() {
    FormStatus.put(route('Customer.updateStatus', { id: FormStatus.slug }), {
        preserveState: false,
        onError: (err) => {
            console.log(err);
        },
...
<template>
    ...
    <AuthenticatedLayout>
                            ...
                            <table class="w-full divide-y divide-gray-200">
                                ...
                                <tbody class="divide-y divide-gray-200" v-if="data.data.length > 0">
                                    <tr v-for="(item, index) in data.data" :key="item.id"
                                        ...
                                        <td class="px-2 py-1 md:px-4 md:py-3  text-xs font-medium text-gray-800">
                                            <span @click="showPembayaranModal(item)" :class="statusClass(item.status)"
                                                class="inline-flex items-center justify-center cursor-pointer rounded-full px-2.5 py-1 ">
                                                ...
                                                <Dropdown align="top" width="48">
                                                    ...
                                                    <template #content>
                                                        <DropdownLink v-if="crud.edit"
                                                            :href="route('Customer.edit', { slug: item.id })"
                                                        ...
                                                        <DropdownLink v-if="crud.show"
                                                            :href="route('Customer.show', { slug: item.id })"
                                                            class="flex justify-start gap-3">
                                                        ...
                                                        <DropdownLink v-if="crud.reset_password"
                                                            :href="route('Customer.reset.password', { slug: item.user.id })"
                                                            class="flex justify-start gap-3">
                                                            <font-awesome-icon class="text-blue-500 hover:text-blue-700"
                                                                :icon="['fas', 'key']" />
                                                            Reset Password
        ...
        <Modal :show="UpdateStatusCustomer">
            <div class="bg-white p-4">
                <div class="p-2 mb-3 rounded-md bg-gray-200">
                    <h3 class="text-lg tracking-wide font-semibold">Form Update Status Customer</h3>
                </div>
                <form @submit.prevent="submitStatus()" class="w-full max-w-full flex flex-col space-y-4 justify-center">
                    <div class="relative w-full">
                        <InputLabel class="w-full" value="Update Status Pembayaran" />
                        <select id="order" v-model="FormStatus.status"
                            ...
                            <option value="2">Pelanggan Lama</option>
                        </select>
