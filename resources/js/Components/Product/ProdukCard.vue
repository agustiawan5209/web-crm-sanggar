...
<template>
    ...
    <div class="relative my-2 md:m-10 w-full max-w-xs overflow-hidden rounded-lg bg-white shadow-md">
        <Link :href="route('Customer.Pembayaran.show', { slug: produk.pembayaran.id })"
            v-for="(image, index) in produk.produk_id.image">
        <img class="h-60 rounded-t-lg object-cover" v-if="image.status" :src="image.image_url" alt="product image" />
        <img class="h-60 rounded-t-lg object-cover" v-else-if="index == 0" :src="image.image_url" alt="product image" />
        ...
        <div class="mt-4 px-5 pb-5">
            ...
            <div class="bg-gray-100 p-1.5 my-2">
                <ul class="list-item space-y-3">
                    ...
                    <li class="text-base font-bold text-slate-700">Tanggal Pengembalian: {{ produk.tgl_pengembalian }}
                    </li>
                    <li class="text-base font-bold text-slate-700">Status Sewa: <span
                            class="p-2 bg-green-100 text-green-800">{{ produk.status }}</span></li>
                </ul>

            </div>
            <button type="button" v-if="produk.status == 'SELESAI' && produk.review == null"
                @click="openModal(produk.produk_id.id, produk.id)"
            ...
            <div class="" v-if="produk.review != null">
                <font-awesome-icon :icon="['fas', 'star']" v-for="star in produk.review.rating" :key="star"
                    class="h-8 w-8 cursor-pointer text-yellow-400" />
            </div>
        </div>
    </div>
</template>
